...
async function runHairColorViaPythonCli(imageBase64, targetHex, strength = 1, options = {}) {
  ...
  const requestedBin = process.env.HAIR_COLOR_PYTHON_BIN && process.env.HAIR_COLOR_PYTHON_BIN.trim();
  const pythonCandidates = requestedBin ? [requestedBin] : ['python3', 'python', 'python3.11', 'python3.10'];
  const timeoutMs = Number(process.env.HAIR_COLOR_PYTHON_TIMEOUT_MS || 90000);
  const argsBase = [cliPath, '--hex', targetHex, '--strength', String(Math.max(0, Math.min(1, Number(strength) || 0)))];
  const labelText = options && options.label;
  if (labelText) {
    argsBase.push('--label', String(labelText));
  }
  ...
  for (const pythonBin of pythonCandidates) {
    try {
      const normalized = await new Promise((resolve, reject) => {
        const child = spawn(pythonBin, argsBase, { stdio: ['pipe', 'pipe', 'pipe'] });
        let stdout = '';
        let stderr = '';
        ...
        child.on('close', (code) => {
          ...
          try {
            ...
            const cleaned = String(parsed.imageOutBase64).replace(/^data:image\/\w+;base64,/, '').replace(/[\r\n\s]+/g, '');
            if (!cleaned) {
      ...
      if (code === 'ENOENT' || /not found/i.test(message)) {
        console.warn('[hair-color] python executable not found', { pythonBin });
        continue;
      }
      console.warn('[hair-color] python cli attempt failed', { pythonBin, error: message });
    }
  }
  ...
  throw new Error('Python hair color CLI tidak tersedia');
}
...
async function runHairColorViaReplicate(imageBase64, targetHex, strength = 1, options = {}) {
  ...
  const editStrength = 0.35 + normalizedStrength * 0.5;
  ...
  const prompt = [
    ...
    `Target hair color: ${colorDescriptor}.`,
  ...
  const inputPayload = {
    ...
    strength: Number(editStrength.toFixed(2)),
